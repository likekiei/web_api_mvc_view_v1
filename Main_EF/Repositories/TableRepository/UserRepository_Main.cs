...
namespace Main_EF.Repositories.TableRepository
{
    ...
    /// 【Main】使用者 Repository
    /// </summary>
    public class UserRepository_Main : GenericRepository_Main<User>, IUserRepository
    {
        public UserRepository_Main(DbContext_Main dbContext) : base(dbContext)
        {
        ...
        /// <returns></returns>
        public void WhereQueryable(ref IQueryable<User> dbData, User_Filter input)
        {
            // 公司
        ...
        /// <param name="input">查詢條件</param>
        /// <returns></returns>
        public User_DTO? GetDTO(User_Filter input)
        {
            // 語法命令
            ...
            // 取出資料
            var result = dbData.Select(x => new User_DTO
            {
                Id = x.Id,
                No = x.No,
                Name = x.Name,
        ...
        /// <returns></returns>
        public List<SelectItemDTO> GetDropList(User_Filter input, ref Pageing_DTO pageingDTO, string defaultItemText)
        {
        ...
        public List<string> GenNos(DateTime date, int count)
        {
            ...
            var numberPrefix = E_NumberPrefix.使用者.EM_GetEnumDescription(); // 代號前綴
