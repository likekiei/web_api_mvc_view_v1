...
namespace ERP_APP.Service.S_Product_Inspection
{
    ...
    /// 送檢單_自定義
    /// </summary>
    public class MF_TI_Z_Service_Erp
    {
        /// <summary>
        /// 送檢單_自定義table MF_TI_Z
        /// </summary>
                #region == 【DI注入用宣告】 ==
        /// <summary>
        /// 資料庫工作單元
        ...
        {
            var check = _MF_TI_Z_Repository.GetAlls(x => x.BBNUM == no).Any();
            return check;
        }
        #endregion
        ...
        /// <param name="no">單號</param>
        /// <returns>[true：存在]</returns>
        public bool Check_IsExist_MF_TI_Z_TI_NO(string no)
        {
            var check = _MF_TI_Z_Repository.GetAlls(x => x.TI_NO == no).Any();
            return check;
        }
        #endregion
